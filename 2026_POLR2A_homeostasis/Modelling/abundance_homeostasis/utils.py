...
def calculate_time_averages(data, treatment_codes=None):
    ...
    for code in treatment_codes:
        ...
        avg = subset.groupby('TIME_HR').agg({
...
def plot_data_preview(data_averaged, treatment_codes=None):
    ...
    for ax, code in zip(axes, treatment_codes):
        ...
        # Plot data
        ax.scatter(data['time'], data['rfp'], color=colors['rfp'], 
                   alpha=0.6, s=60, label='RFP', zorder=3)
        ax.scatter(data['time'], data['gfp'], color=colors['gfp'], 
                   alpha=0.6, s=60, marker='s', label='GFP', zorder=3)
