...
def process_single_site(input_file,input_dir,label_image_dir,features_dir,illumcorr_file):
    ...
    features_cell = features[0]
    features_nuc = features[1]
    features_cyto = features[2]
    ...
    features_cyto.to_csv(features_dir / Path("cyto_" + Path(input_file).stem + ".csv"), index=False)
