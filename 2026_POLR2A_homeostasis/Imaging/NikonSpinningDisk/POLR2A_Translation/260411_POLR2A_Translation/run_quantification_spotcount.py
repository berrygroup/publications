...
def process_single_site(input_file,input_dir,label_image_dir,fish_spots_1_dir,fish_spots_2_dir,features_dir,illumcorr_file):
    ...
    # quantify fluorescence intensity per channel for each spot
    # filter for centre 1500 sq pixels of image, minimising illumination bias and chromatic aberration
    # only do for ATTO633 channel
    spots = spots2

    mask = (spots[:, 1] > 352) & (spots[:, 1] < 1952) & (spots[:, 2] > 352) & (spots[:, 2] < 1952)
    spots_filtered = spots[mask]

    spots_binary = np.zeros_like(cell_labels)
    spots_binary[spots_filtered[:,0],spots_filtered[:,1],spots_filtered[:,2]] = 1

    spots_label = measure.label(spots_binary)

    spots_label = AICSImage(spots_label[np.newaxis,np.newaxis,:,:,:],
                                channel_names = ["Spots"],
                                physical_pixel_sizes=intensity_image.physical_pixel_sizes)
    

    cell_label_per_pixel_spot = quantify(labels, spots_label, point_objects="Spots")
    
    quantify_per_spot = quantify(intensity_image_corrected, spots_label, point_objects="Spots")
    
    spot_quantification = cell_label_per_pixel_spot[0].merge(quantify_per_spot[0], on="label", how="left")

    spot_quantification.to_csv(features_dir / Path("spots_633_" + Path(input_file).stem + ".csv"), index=False)
