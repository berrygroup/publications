...
Imaging performed on Nikon Ti2 with CSU-W1 spinning disk using 20x air objective, automated via JOBS. Z-stacks contain 12 images at a spacing of 2µm.
...
model_fits <- tibble(summary(emm_out)$emtrends) %>%
  mutate(timepoint_h.trend=-timepoint_h.trend,
         ...
         timepoint_h.trend_relative_error=(timepoint_h.trend - lower.CL)/timepoint_h.trend) %>%
  select(-c(upper.CL_new,lower.CL_new)) %>%
  mutate(recovery_half_life_estimate=log(2)/timepoint_h.trend,
         recovery_half_life_error=recovery_half_life_estimate*timepoint_h.trend_relative_error,
         recovery_half_life_lower.CL=recovery_half_life_estimate-recovery_half_life_error,
         recovery_half_life_upper.CL=recovery_half_life_estimate+recovery_half_life_error) %>%
  mutate(siRNA=factor(siRNA,levels=c("Scrambled","ARMC5")))
model_fits
...
cell_count_model_fits <- tibble(summary(emm_out_bleach)$emtrends) %>%
  mutate(dilution_rate=timepoint_h.trend,
         dilution_rate_relative_error=(timepoint_h.trend-lower.CL)/timepoint_h.trend,
         cell_cycle_duration=log(2)/dilution_rate,
         cell_cycle_duration_error=dilution_rate_relative_error*cell_cycle_duration,
         cell_cycle_duration_lower.CL=cell_cycle_duration-cell_cycle_duration_error,
         cell_cycle_duration_upper.CL=cell_cycle_duration+cell_cycle_duration_error) %>%
...
We convert this to an error in half life, keeping the same relative error.
...
combined_model_fits <- cell_count_model_fits %>% 
  ...
  mutate(timepoint_h.trend_degradation = timepoint_h.trend_removal - timepoint_h.trend_dilution,
         ...
         timepoint_h.trend_degradation_error_relative = timepoint_h.trend_degradation_error / timepoint_h.trend_degradation) %>%
  mutate(corrected_half_life=log(2)/timepoint_h.trend_degradation,
         corrected_half_life_error=timepoint_h.trend_degradation_error_relative*corrected_half_life,
         corrected_half_life_lower.CL=corrected_half_life-corrected_half_life_error,
         corrected_half_life_upper.CL=corrected_half_life+corrected_half_life_error)
...
combined_model_fits %>%
  ...
  geom_point(aes(col=siRNA),size=0.8,pch=21,fill="white",stroke=0.5) + 
...
Change in degradation rate of `r c` would be predicted to give a change in protein concentration of $1/c$ = `r expected_change` $\pm$ `r round(expected_change_error,2)`.
