...
filter(to_plot_IF,
  ...
  theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5),
        ...
        legend.title = element_blank(),
        legend.key.size = unit(3,"mm"))
ggsave(filename = "PLOTS/mCherry_barplots_IF_by_siRNA_subset_total_ser5P.pdf",width=3.2,height=2)
...
p <- dat_non_border %>%
  filter(Primary=="pSer5 Pol II" & imaging_date=="20240328") %>%
  filter(siRNA %in% c("ARMC5 pool","ARMC5+INTS8","INTS8 pool","Scrambled")) %>%
  ggplot(aes(y=Nuclei_area,x=Nuclei_intensity_mean_488,col=siRNA,group=interaction(well_name,siRNA,imaging_date))) +
  geom_point(alpha=0.1) +
  ggtitle("20240328")
ggExtra::ggMarginal(p,groupColour = T,groupFill = T)

p <- dat_non_border %>%
  filter(Primary=="pSer5 Pol II" & imaging_date=="20240404") %>%
  filter(siRNA %in% c("ARMC5 pool","ARMC5+INTS8","INTS8 pool","Scrambled")) %>%
  ggplot(aes(y=Nuclei_area,x=Nuclei_intensity_mean_488,col=siRNA,group=interaction(well_name,siRNA,imaging_date))) +
  geom_point(alpha=0.1) +
  ggtitle("20240404")
...
p <- dat_non_border %>%
  filter(Primary=="pSer5 Pol II" & imaging_date=="20240701") %>%
  filter(siRNA %in% c("ARMC5 pool","ARMC5+INTS8","INTS8 pool","Scrambled")) %>%
  ggplot(aes(y=Nuclei_area,x=Nuclei_intensity_mean_488,col=siRNA,group=interaction(well_name,siRNA,imaging_date))) +
  geom_point(alpha=0.1) +
  ggtitle("20240701")
...
dat_non_border %>%
  filter(Primary=="pSer5 Pol II" ) %>%
    filter(siRNA %in% c("ARMC5 pool","ARMC5+INTS8","INTS8 pool","Scrambled")) %>%
  ggplot(aes(x=siRNA,y=Nuclei_intensity_mean_488,col=siRNA,group=well_name)) + geom_boxplot() +
  facet_wrap(~imaging_date, scales = "free") +
  theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5))
