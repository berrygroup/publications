...
scatter <- dat_non_border %>%
  filter(imaging_date=="20240701") %>%
  filter(siRNA %in% c("INTS8 pool","Scrambled") & Cell=="HCT116 mCherryPOLR2A" & Primary=="pSer5 Pol II") %>%
  ggplot(aes(x=Nuclei_area,y=Nuclei_intensity_mean_488,col=siRNA)) +
  geom_point(pch=".",alpha=0.1) +
  geom_smooth(aes(group=interaction(well_name,siRNA)))

ggExtra::ggMarginal(scatter,group=TRUE)
