---
title: "EU in HCT116 cells with ARMC5 KD"
author: "Scott Berry"
date: "`r Sys.Date()`"
...
- `illumination_correction_EU_EU.ipynb`
- `save_example_images_EU_EU.ipynb` (used to generate example images and training data for the cellpose model)
- `run_quantification_EU.py` (parallelised using e.g. `PBS/batch_quantify_EU_240628.pbs`)
...
acquisition_dirs <- purrr::map_chr(
  c("ARMC5_KD_EU/240628_ARMC5_EU/Cycle2_EU/20240701_175159_151",
    "ARMC5_KD_EU/240705_ARMC5_EU/Cycle2_EU/20240708_171627_050",
    "ARMC5_KD_EU/240713_ARMC5_EU/Cycle2_EU/20240715_173604_562"),~file.path(data_dir,.))
...
dat %>% 
  distinct(cell,plating_date,imaging_date,well_name,siRNA,EU) %>%
  group_by(cell,plating_date,imaging_date,siRNA,EU) %>%
  count() %>%
  ungroup() %>%
  ggplot(aes(x=cell,y=n,fill=siRNA)) + geom_col(position = "dodge") + facet_grid(EU~imaging_date) +
  coord_flip() +
...
# update 25/7/24 for EU in HCT116

bg_647 <- mean(c(142.125,144.4,142))
bg_405 <-mean(c(174.8,191.2,179))
...
dat_non_border_sample_longer <- dat_non_border %>%
  ...
  select(Nuclei_centroid_0,Nuclei_centroid_1,Nuclei_intensity_mean_405,Nuclei_intensity_mean_647) %>%
...
dat_non_border %>% 
  ggplot(aes(x=siRNA,y=Nuclei_intensity_mean_647,col=siRNA,group=interaction(well_name,siRNA))) + 
  #geom_boxplot(outlier.shape=NA) + 
  geom_boxplot() + 
  facet_grid(EU~imaging_date) +
...
field_averages <- dat_non_border %>%
  group_by(image_dir,filename_ome_tiff,EU,siRNA,imaging_date,well_name,field_id) %>%
  summarise(Nuclei_intensity_mean_647_mean=mean(Nuclei_intensity_mean_647),.groups = 'drop') 

outlier_fields <- field_averages %>%
  group_by(image_dir,EU,siRNA,imaging_date) %>%
  summarise(across(Nuclei_intensity_mean_647_mean,c(p25=~quantile(.,0.25),p75=~quantile(.,0.75))),.groups = 'drop') %>%
  left_join(field_averages,by = join_by(image_dir, EU, siRNA, imaging_date)) %>%
  mutate(Nuclei_intensity_mean_647_mean_IQR = Nuclei_intensity_mean_647_mean_p75 - Nuclei_intensity_mean_647_mean_p25,
         outlier = case_when(
           Nuclei_intensity_mean_647_mean > Nuclei_intensity_mean_647_mean_p75 + 1.5*Nuclei_intensity_mean_647_mean_IQR ~ TRUE,
           Nuclei_intensity_mean_647_mean < Nuclei_intensity_mean_647_mean_p25 - 1.5*Nuclei_intensity_mean_647_mean_IQR ~ TRUE,
           TRUE ~ FALSE))

outlier_fields %>%
  ggplot(aes(y=Nuclei_intensity_mean_647_mean,x=siRNA,col=outlier)) +
  geom_jitter() +
  scale_y_log10() +
  facet_wrap(EU~imaging_date)

outlier_fields %>%
  count(imaging_date,EU,siRNA,outlier) %>%
  group_by(imaging_date,EU,siRNA) %>%
  mutate(freq=n/sum(n)) %>%
  filter(outlier=="TRUE")
...
dat_non_border <- dat_non_border %>%
  anti_join(filter(outlier_fields,outlier==TRUE),by = join_by(image_dir, well_name, field_id, filename_ome_tiff, imaging_date, siRNA, EU))
...
dat_non_border %>%
  mutate(well_row = factor(str_extract(well_name, "[A-Z]"),levels=LETTERS),
         well_column = as.integer(as.character(str_extract(well_name, "[0-9]+")))) %>%
  distinct(imaging_date,well_name,well_row,well_column,EU) %>%
  ggplot(aes(x=well_column,y=well_row,fill=EU)) +
  geom_tile() +
  scale_y_discrete(limits=rev) +
  facet_wrap(~imaging_date) +
...
axis_rescaler <- filter(overall_summaries,siRNA=="Scrambled") %>% pull(mean_Nuclei_intensity_mean_647_plate_normalised)

no_EU_well_summaries <- dat_non_border %>%
  filter(EU=="No EU") %>%
  left_join(normalisation_factors, by = join_by(imaging_date)) %>%
  mutate(Nuclei_intensity_mean_647_plate_normalised = Nuclei_intensity_mean_647 * plate_normaliser) %>%
  group_by(imaging_date,well_name,siRNA) %>%
...
no_EU_overall_summaries <- no_EU_well_summaries %>%
  group_by(siRNA) %>%
  summarise(mean_Nuclei_intensity_mean_647 = mean(Nuclei_intensity_mean_647),
            sd_Nuclei_intensity_mean_647 = sd(Nuclei_intensity_mean_647),
            mean_Nuclei_intensity_mean_647_plate_normalised = mean(Nuclei_intensity_mean_647_plate_normalised),
...
with_EU <- well_summaries %>%
  ggplot(aes(x=siRNA,y=Nuclei_intensity_mean_647_plate_normalised/axis_rescaler,col=siRNA)) +
  geom_col(data = overall_summaries, aes(y=mean_Nuclei_intensity_mean_647_plate_normalised/axis_rescaler,fill=siRNA),col=NA,alpha=0.4,width=0.8) +
  geom_point(data = overall_summaries, aes(y=mean_Nuclei_intensity_mean_647_plate_normalised/axis_rescaler)) +
  geom_errorbar(data = overall_summaries, aes(y=mean_Nuclei_intensity_mean_647_plate_normalised/axis_rescaler,
                                              ymin=(mean_Nuclei_intensity_mean_647_plate_normalised-sd_Nuclei_intensity_mean_647_plate_normalised)/axis_rescaler,
                                              ymax=(mean_Nuclei_intensity_mean_647_plate_normalised+sd_Nuclei_intensity_mean_647_plate_normalised)/axis_rescaler),width=0.25) +
  geom_jitter(width=0.25,pch=4,alpha=0.3) +
  scale_y_continuous(limits = c(0,1.4),name="Mean nuclear\nintensity",expand = c(0,0)) +
  theme_bw(base_size = 8) + 
...
no_EU <- no_EU_well_summaries %>%
  ggplot(aes(x="No EU",y=Nuclei_intensity_mean_647_plate_normalised/axis_rescaler,col=siRNA)) +
  geom_col(data = no_EU_overall_summaries, aes(y=mean_Nuclei_intensity_mean_647_plate_normalised/axis_rescaler,fill=siRNA),col=NA,alpha=0.4,width=0.8) +
  geom_point(data = no_EU_overall_summaries, aes(y=mean_Nuclei_intensity_mean_647_plate_normalised/axis_rescaler)) +
  geom_errorbar(data = no_EU_overall_summaries, aes(y=mean_Nuclei_intensity_mean_647_plate_normalised/axis_rescaler,
                                              ymin=(mean_Nuclei_intensity_mean_647_plate_normalised-sd_Nuclei_intensity_mean_647_plate_normalised)/axis_rescaler,
                                              ymax=(mean_Nuclei_intensity_mean_647_plate_normalised+sd_Nuclei_intensity_mean_647_plate_normalised)/axis_rescaler),width=0.25) +
  geom_jitter(width=0.25,pch=4,alpha=0.3) +
  scale_y_continuous(limits = c(0,1.4),name="Mean nuclear\nEU intensity",expand = c(0,0)) +
  theme_bw(base_size = 8) + 
...
with_EU + no_EU + plot_layout(widths=c(2,1),guides="collect")

ggsave("PLOTS/EU_barplots_by_siRNA.pdf",width=6,height=4,units="cm")
...
write_csv(x = well_summaries,file = "SUMMARIES/siRNA_EU_well_summaries.csv")
write_csv(x = overall_summaries,file = "SUMMARIES/siRNA_EU_overall_summaries.csv")
